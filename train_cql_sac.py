...
class DiscreteCAL_QL:
    ...
    def _q_loss(
        ...
        with torch.no_grad():
            ...
            if self.cql_max_target_backup:
                ...
                q1_next_val = self.target_critic_1.get_all(next_adj, next_features, next_candidate, graph_pool, next_action_masks)
                q2_next_val = self.target_critic_2.get_all(next_adj, next_features, next_candidate, graph_pool, next_action_masks)
                ...
                next_q_values = torch.min(
                    self.target_critic_1.get_all(next_adj, next_features, next_candidate, graph_pool, next_action_masks),
                    self.target_critic_2.get_all(next_adj, next_features, next_candidate, graph_pool, next_action_masks),
                )
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        policy_loss = self._policy_loss(
            adj, features, candidates, mb_g_pool, actions, action_masks, action_probs, new_actions, log_action_probs, alpha, log_pi
        )
...
def train(config: TrainConfig):
    ...
    print("---------------------------------------")
    print(f"Training CAL-QL, Env: n_jobs {config.n_jobs}, n_machines {config.n_machines}, Seed: {seed}")
    print("---------------------------------------")
