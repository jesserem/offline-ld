...
class TrainConfig:
    # Experiment
    device: str = "cpu"
    # env: str = "LunarLander-expert-v2"  # OpenAI gym environment name
    # env: str = "door-expert-v1"  # OpenAI gym environment name
    # instance: str = "ta31"  # JSSP Instance
    n_jobs: int = 20 # Number of jobs
    n_machines: int = 20 # Number of machines
    eval_instances: str = f"" #eval instances
    ...
    target_update_period: int = 5000  # Frequency of target nets updates
    ...
    q_dropout: float = 0.2  # Dropout in actor network
    ...
    # Wandb logging
    project: str = "JSSP-Offline-Test"
    group: str = f"{n_jobs}x{n_machines}"
    name: str = "QRDQN-CQL-" + "-" + dataset

    # Evaluation
    path_eval: str = "../eval_data"
    save_path_checkpoint: str = "../analyse_results/dqn_cql_alpha_1/6_6/noisy"
    save_folder_results: str = "./results_exp/results_eval_dqn_cql/"
...
class DiscreteCQL_DQN:
    ...
    def _target_q_values(self, next_adj: torch.Tensor, next_features: torch.Tensor, next_candidates: torch.Tensor,
        ...
        with torch.no_grad():
            ...
            q_target_values = target_q_values_all.gather(1, action_idx.reshape(batch_size, 1, 1).expand(batch_size, 1,
                                                                                                        self.N)).transpose(
                1, 2)
...
def eval(config: TrainConfig, eval_instance_arr: np.ndarray, state_dict_path: str):
    ...
    print("---------------------------------------")
    print(f"Training CQL-DQN, Env: n_jobs {config.n_jobs}, n_machines {config.n_machines}, EVAL")
    print("---------------------------------------")
