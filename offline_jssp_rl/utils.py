...
def combine_offline_online_batch(online_batch: TensorBatch, offline_batch: TensorBatch) -> TensorBatch:
    (
        ...
        online_rewards,
        online_next_adj,
        online_next_features,
        online_next_candidates,
        online_next_action_masks,
        online_dones,
        # online_mc_returns
    ) = online_batch
    (
        ...
        offline_candidates,
        offline_actions,
        offline_action_masks,
        offline_rewards,
        offline_next_adj,
        offline_next_features,
        offline_next_candidates,
        offline_next_action_masks,
        offline_dones,
        # offline_mc_returns
    ) = offline_batch
    adj_stack = torch.vstack([online_adj, offline_adj])
    # print("adj_stack shape", adj_stack.shape)
    adj = aggr_obs(adj_stack.to_sparse(), adj_stack.shape[1])
    # print("adj shape", adj.shape)
    online_features = online_features.reshape(-1, online_features.size(-1))
    offline_features = offline_features.reshape(-1, offline_features.size(-1))
    features = torch.vstack([online_features, offline_features])
    next_adj_stack = torch.vstack([online_next_adj, offline_next_adj])
    next_adj = aggr_obs(next_adj_stack.to_sparse(), next_adj_stack.shape[1])
    next_online_features = online_next_features.reshape(-1, online_next_features.size(-1))
    next_offline_features = offline_next_features.reshape(-1, offline_next_features.size(-1))
    next_features = torch.vstack([next_online_features, next_offline_features])
    candidates = torch.vstack([online_candidates, offline_candidates])
    actions = torch.vstack([online_actions, offline_actions])
    action_masks = torch.vstack([online_action_masks, offline_action_masks])
    rewards = torch.vstack([online_rewards, offline_rewards])
    next_candidates = torch.vstack([online_next_candidates, offline_next_candidates])
    next_action_masks = torch.vstack([online_next_action_masks, offline_next_action_masks])

    dones = torch.vstack([online_dones, offline_dones])
    # mc_returns = torch.vstack([online_mc_returns, offline_mc_returns])
    # print(rewards)
