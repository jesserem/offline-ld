...
class FJSPEnvForSameOpNums(gym.Env):
    """
    ...
    def __init__(self, n_j, n_m, device):
        ...
        self.n_operations = n_j * n_m
        self.fea_j = None
        self.op_mask = None
        self.fea_m = None
        self.mch_mask = None
        self.dynamic_pair_mask = None
        self.comp_idx = None
        self.candidate = None
        self.fea_pairs = None
        ...
        self.observation_space = Dict({
            "fea_j": Box(low=-np.inf, high=np.inf, shape=(1, self.n_operations, self.op_fea_dim), dtype=np.float64),
            "op_mask": Box(low=0, high=1, shape=(1, self.n_operations, 3), dtype=np.float32),
            "fea_m": Box(low=-np.inf, high=np.inf, shape=(1, self.number_of_machines, self.mch_fea_dim), dtype=np.float64),
            "mch_mask": Box(low=0, high=1, shape=(1, self.number_of_machines, self.number_of_machines), dtype=bool),
            "dynamic_pair_mask": Box(low=0, high=1, shape=(1, self.number_of_jobs, self.number_of_machines), dtype=bool),
            "comp_idx": Box(low=0, high=1, shape=(1, self.number_of_machines, self.number_of_machines, self.number_of_jobs), dtype=np.float32),
            "candidate": Box(low=0, high=self.n_operations, shape=(1, self.number_of_jobs), dtype=np.int64),
            "fea_pairs": Box(low=0, high=1, shape=(1, self.number_of_jobs, self.number_of_machines, 8), dtype=np.float64),
        })

        self.action_space = Discrete(self.n_operations)
    ...
    def step(self, action):
        ...
        return self.state, reward[0], self.done()[0], False, {}
