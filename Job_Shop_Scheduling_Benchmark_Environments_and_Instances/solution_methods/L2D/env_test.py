...
class NipsJSPEnv_test(gym.Env):
    def __init__(self, n_j: int, n_m: int):
        ...
        self.observation_space = Dict({
            "adj": Box(low=0, high=1, shape=(self.number_of_tasks, self.number_of_tasks), dtype=np.single),
    ...
    def reset(self, JSM_env: JobShop = None, seed = None, options=None):
        super().reset(seed=seed)
        if JSM_env is None:
            self.JobShopModule = options["JSM_env"]
        else:
        ...
        self.initQuality = self.JSM_LBs.max()
        fea = np.concatenate((self.JSM_LBs.reshape(-1, 1),
        ...
        adj = self.JSM_adj.copy()

        mask = self.JSM_mask.copy()
        omega = self.JSM_omega.copy()
        state = {"adj": adj, "fea": fea, "omega": omega, "mask": mask}
    ...
    @property
    def make_span(self):
        return self.JobShopModule.makespan
    ...
    def step(self, action):
        ...
        # prepare for return
        feature_JSM = np.concatenate((self.JSM_LBs.reshape(-1, 1),
                              self.JSM_finished_mark.reshape(-1, 1)), axis=1)
        ...
        mask = self.JSM_mask.copy()
        omega = self.JSM_omega.copy()
        state = {"adj": adj, "fea": feature_JSM, "omega": omega, "mask": mask}
        # print("action env: ", action)
        return state, reward_JSM, self.done(), False, {}
